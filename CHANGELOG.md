...
- **Prefix cache metrics** — `cacheReadInputTokens` is now populated in `Usage` for text, streaming, and structured responses when Workers AI returns `prompt_tokens_details.cached_tokens`. Works with both Prism PHP and Laravel AI SDK gateways. Pair with session affinity for multi-turn cache hits.
...
- 104 tests, 218 assertions (was 98/204)

## v0.4.5 — 2026-04-13

- **Support prism-php/prism ^0.100 and laravel/ai ^0.5** — Widened dependency constraints for latest upstream releases.

## v0.4.4 — 2026-04-10

- **Reasoning exhaustion safeguards** — `ThinkingCompleteEvent` is now emitted even when reasoning exhausts `max_tokens` before producing content. Accumulated thinking is included in `StreamEndEvent.additionalContent['thinking']`.
